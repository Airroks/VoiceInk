...
Maus-Shortcut (F9): Taste 6 wurde im nativen Shortcut-Recorder nicht erkannt — **auch mit deaktivierter BTT-Zuweisung und ohne Logitech Options** (Alexander hat beides ausgeschlossen; die erste Vermutung „BTT schluckt den Klick" war falsch). Ursache offen, nicht weiter untersucht. Alexander bleibt bei BTT→F20 mit F20 als VoiceInk-Toggle — funktioniert wie zuvor.
...
Status: geloest extern (2026-08-06, via BetterTouchTool; bestaetigt 2026-09-20) · nativ seit Upstream 2.20 vorhanden, **erkennt Maustaste 6 aber nicht** — Test 2026-09-20 mit deaktiviertem BTT und ohne Logitech Options negativ, Ursache offen
DoD: entfaellt, solange BTT→F20 im Alltag traegt. Falls nativ gewuenscht: [ ] Ursache klaeren (Ansatz: `log stream --predicate 'process == "VoiceInk"'` waehrend der Aufnahme im Recorder — kommt ueberhaupt ein `otherMouseDown` mit buttonNumber 5 an? Lokale NSEvent-Monitore sehen Maus-Events nur, wenn der Zeiger ueber einem VoiceInk-Fenster liegt) [ ] Maustaste im Recorder gesetzt [ ] Halten + Toggle verifiziert
