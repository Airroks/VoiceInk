...
final class UpdaterViewModel: NSObject, ObservableObject, SPUUpdaterDelegate {
    ...
    override init() {
        ...
        // Fork guard: the bundled Sparkle feed points at the upstream appcast, so
        // installing an update replaces this personal build with the official app
        // and silently drops every fork change. Local builds never touch the
        // updater (it stays lazily uninstantiated) — upstream is merged by hand
        // and rebuilt via `make local`.
        #if LOCAL_BUILD
