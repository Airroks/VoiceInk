...
final class ShortcutMonitor {
    ...
    private func releasePressedShortcuts(eventTime: TimeInterval) {
        for action in Array(shortcuts.keys) {
            ...
            if shouldDispatchUp {
                // Fork diagnostics: keep a visible trace when a release had to be
                // synthesized (see F1 notes in docs/FEATURES.md).
                logger.warning(
                    "Synthesizing shortcut release after tap interruption for action: \(String(describing: action), privacy: .public)"
                )
